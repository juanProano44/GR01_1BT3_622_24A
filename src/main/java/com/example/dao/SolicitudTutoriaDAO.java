...
public class SolicitudTutoriaDAO {
    ...
    public List<SolicitudTutoria> getSolicitudesPorTutor(int tutorId) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return session.createQuery("from SolicitudTutoria s where s.tutoria.tutor.id = :tutorId", SolicitudTutoria.class)
    ...
    public SolicitudTutoria getSolicitudByAlumnoAndTutoria(int alumnoId, int tutoriaId) {
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            return session.createQuery("from SolicitudTutoria s where s.alumno.id = :alumnoId and s.tutoria.id = :tutoriaId", SolicitudTutoria.class)
    ...
    public List<SolicitudTutoria> getTutoriasAceptadasPorAlumno(int alumnoId) {
        ...
        try (Session session = HibernateUtil.getSessionFactory().openSession()) {
            ...
            solicitudes = session.createQuery(
                            "FROM SolicitudTutoria WHERE alumno.id = :alumnoId AND estado = 'Aceptada'", SolicitudTutoria.class)
                    .setParameter("alumnoId", alumnoId)
                    .list();
